...
class AppPreferences(context: Context) {
    ...
    val profilesFlow: Flow<List<Profile>> = context.dataStore.data
        .catch { if (it is IOException) emit(emptyPreferences()) else throw it }
        .map { prefs ->
            // Trigger reload when PROFILES_REV changes
            val json = encryptedPrefs.getString("profiles_json", null)
    ...
    suspend fun saveProfiles(list: List<Profile>) {
        withContext(Dispatchers.IO) {
            encryptedPrefs.edit {
                putString("profiles_json", ProfileJson.encodeList(list))
            }
        }
        context.dataStore.edit { prefs ->
    ...
    val profilesSnapshot: Flow<ProfilesSnapshot> = context.dataStore.data
        ...
        .map { prefs ->
            ProfilesSnapshot(
                list = ProfileJson.decodeList(encryptedPrefs.getString("profiles_json", null)),
    ...
    suspend fun getAuthToken(): String = withContext(Dispatchers.IO) {
        encryptedPrefs.getString("auth_token", "") ?: ""
    }
    ...
    suspend fun getConfigUrl(): String = withContext(Dispatchers.IO) {
        encryptedPrefs.getString("config_url", "") ?: ""
    }
    ...
    suspend fun getDecryptionKey(): String = withContext(Dispatchers.IO) {
        encryptedPrefs.getString("decryption_key", "") ?: ""
    }
    ...
    suspend fun getSubscriptionExpiresAt(): Long = withContext(Dispatchers.IO) {
        encryptedPrefs.getLong("sub_expires_at", 0L)
    }
    ...
    suspend fun getWgConfig(): String = withContext(Dispatchers.IO) {
        encryptedPrefs.getString("wg_config", "") ?: ""
    }
